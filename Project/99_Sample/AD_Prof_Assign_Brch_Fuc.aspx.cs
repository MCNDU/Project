...
namespace Project._03_Sample {
    public partial class AD_Prof_Assign_Brch_Fuc : System.Web.UI.Page {
        //連結資料庫，命名為cnStr
        //@"資料來源=(本地資料庫)\"
        ...
        //儲存學生資料用之虛擬表格
        DataTable Student_Data = new DataTable ("Student_Data");

        //Studnet_Data資料查詢&轉換
        //步驟：根據期別查詢->調出資料->代換(二次查詢)->轉送至Student_GV
        //需要：Batch_DDL資料(所選期別為何)，也就是int Batch
        private void Inquire_N_Transform (int Batch) {
            //根據期別查詢
            int Current_Batch = Batch;

            using (SqlConnection cn = new SqlConnection ()) {
                cn.ConnectionString = cnStr;
                //SqlDataAdapter ST_Data = new SqlDataAdapter ("SELECT * FROM ");
            }
        }
